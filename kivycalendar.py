...
class DatePicker(TextInput):
    ...
    def update_value(self, inst):
        ...
        if len(str(self.cal.active_date[1])) == 1:
            month = '0'+(str(self.cal.active_date[1]))
        else:
            month = str(self.cal.active_date[1])

        if len(str(self.cal.active_date[0])) == 1:
            day ='0'+(str(self.cal.active_date[0]))
        else:
            day = str(self.cal.active_date[0])

        self.text = "%s-%s-%s" % tuple([self.cal.active_date[2], month, day])
